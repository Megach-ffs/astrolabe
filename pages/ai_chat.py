...
sl.title("💬 AI Chat & Code Snippets")

# ── Guard ─────────────────────────────────────
if not sl.session_state.get("ai_enabled"):
    sl.warning("⚠️ AI Assistant is not enabled.")
...
if df is not None:
    sl.caption(f"🤖 **Context attached:** Dashboard dataset ({len(df)} rows × {len(df.columns)} columns)")
else:
    sl.caption("🤖 **Context:** No dataset loaded.")
...
if prompt := sl.chat_input("Ask a question or request a pandas snippet...") or initial_prompt:
    ...
    with c1:
        if sl.button("🗑️ Clear Chat", key="clear_chat"):
            sl.session_state.ai_chat_messages = []
            sl.rerun()
